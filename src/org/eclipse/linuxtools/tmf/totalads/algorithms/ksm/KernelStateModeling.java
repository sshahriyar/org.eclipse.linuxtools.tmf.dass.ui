...
public class KernelStateModeling implements IDetectionAlgorithm {
    ...
    private String[] fTrainingOptions = { "Kernel:2.6.35-3.2.x", "true", "Kernel:3.5-3.13", "false" };
    private String[] fTestingOptions = { "Alpha", "0.0" };
        ...
        if (trainingSettings != null) {
            ...
            for (int count = 1; count < fTrainingOptions.length; count += 2) {
                if (trainingSettings[count].equals("true")) {
                    fTrainingOptions[count] = "true";
                    trueCount++;
                    if (trueCount > 1) {
                        fTrainingOptions[count] = "false";
                        throw new TotalADSGeneralException("Please, select only one option as true.");
                    }
                }
                else if (trainingSettings[count].equals("false")) {
                    fTrainingOptions[count] = "false";
                } else {
    ...
    public void validate(ITraceIterator trace, String database, IDataAccessObject dataAccessObject, Boolean isLastTrace, IAlgorithmOutStream outStream) throws TotalADSGeneralException, TotalADSDBMSException, TotalADSReaderException {
        ...
        while (fAlpha < fMaxAlpha) {
            ...
            fAlpha += 0.02;
            outStream.addOutputEvent("Increasing fAlpha to " + fAlpha);
            outStream.addNewLine();
        ...
        if (isLastTrace) {
            ...
            outStream.addOutputEvent("Database updated with final fAlpha: " + fAlpha);
            outStream.addNewLine();
            Double anomalyPercentage = (fValidationAnomalyCount.doubleValue() / fValidationTraceCount) * 100;

            outStream.addOutputEvent("Anomalies at fAlpha " + fAlpha + " are " + anomalyPercentage);
            outStream.addNewLine();
            outStream.addOutputEvent("Total traces " + fValidationTraceCount);
            outStream.addNewLine();
        }
    ...
    private Boolean evaluateKSM(Double alpha, TraceStates testStates, IDataAccessObject dataAccessObject, String database) throws TotalADSDBMSException {
        ...
        String maxVal = dataAccessObject.selectMax("FS", database, TRACE_COLLECTION);
        ...
        if (testStates.FS > maxFS) {
            ...
            for (double incr = testStates.FS; incr <= maxFS; incr += 0.01) {
                ...
                // which match FS=incr
               try (IDBCursor cursor = dataAccessObject.select("FS", null, incr,
                       database, TRACE_COLLECTION)){
                        if (cursor.hasNext()) {
                            ...
                            // break because we don't want to increment incr further as
                            // the trace is normal
                            break;
                        }// end if
               }
    ...
    private void saveTraceData(IDataAccessObject dataAccessObject, String database, TraceStates states) throws TotalADSDBMSException {
        ...
        try {
            ...
            dataAccessObject.insertUsingJSON(database, jsonDoc, TRACE_COLLECTION);

            // this also works
            // dataAccessObject.insert(states, database,TRACE_COLLECTION);

        } catch (TotalADSDBMSException ex) {
            if (ex.getMessage() != null && (!ex.getMessage().contains("E11000") || !ex.getMessage().contains("duplicate"))) {
    ...
    private void saveSettingsInDatabase(Double alpha, String database, IDataAccessObject dataAccessObject)
        ...
        String kernel = "";
        for (int count = 1; count < fTrainingOptions.length; count += 2) {
            if (fTrainingOptions[count].equalsIgnoreCase("true")) {
        ...
        dataAccessObject.insertOrUpdateUsingJSON(database, jsonKey, jsonObjToUpdate, this.SETTINGS_COLLECTION);

    }

    /**
     * Gets fAlpha and other settings from the database. Alpha is returned and
    ...
    private Double getSettingsFromDatabase(String database, IDataAccessObject dataAccessObject) throws TotalADSDBMSException {
        ...
        try (IDBCursor cursor = dataAccessObject.select(SettingsCollections.KEY.toString(),
                "", settingsKey, database, SETTINGS_COLLECTION)){
                if (cursor.hasNext()) {
        ...
        for (int count = 0; count < fTrainingOptions.length; count += 2) {
            if (fTrainingOptions[count].equalsIgnoreCase(kernelVersion)) {
                fTrainingOptions[count + 1] = "true";
                // only one option can be true
            } else {
                fTrainingOptions[count + 1] = "false";
            }
        }
    ...
    private void intializeStates() {
        ...
        if (fTrainingOptions[1].equals("true")) {
    ...
    public String getDescription() {
        return " KSM focuses on detection of anomalies by transforming system calls into states of kernel modules. "
                ...
                + " of states from traces, adjust a decision threshold fAlpha, and use this information to evaluate states of"
